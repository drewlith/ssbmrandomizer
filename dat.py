...
def load_data(player_file_data): # Call before doing anything else
    global header
    global data_block
    global file_data
    file_data = player_file_data
    header = file_data[0x0:0x20]
    data_block = file_data[0x20:0x20+data_block_size()]
...
def get_special_attribute_data(special_attribute_block_size):
    return data_block[ft_attributes_end():ft_attributes_end()+special_attribute_block_size]

def get_article_data(fighter):
    articles_datas = []
    for i in range(len(fighter.articles_offsets)):
        articles_datas.append(data_block[fighter.articles_offsets[i]:fighter.articles_offsets[i]+fighter.articles_sizes[i]])
...
class Throw(): # Class for handling Throw Event
    def __init__(self, data, offset):
        self.data = data
        self.offset = offset
        self.name = "" # Assigned later in script
        self.tags = [] # Assigned later in script
